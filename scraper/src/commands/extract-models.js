/**
 * Extract and download boardview files for popular device models
 * Searches known sources and organizes files
 */
...
/**
 * Known boardview file sources
 */
const BOARDVIEW_SOURCES = [
  {
    name: 'AliSaler',
    baseUrl: 'https://www.alisaler.com',
    searchPath: '/category-laptop-boardview/',
    type: 'website'
  },
  {
    name: 'Gadget Manual',
    baseUrl: 'https://www.gadget-manual.com',
    searchPath: '/boardview/',
    type: 'website'
  },
  {
    name: 'Laptop Schematic',
    baseUrl: 'https://laptopschematic.com',
    searchPath: '/boardview/',
    type: 'website'
  },
  {
    name: 'Vinafix',
    baseUrl: 'https://vinafix.com',
    searchPath: '/forums/boardview/',
    type: 'forum'
  }
];
...
async function createDeviceStructure(manufacturer, model) {
  ...
  const basePath = path.join(process.cwd(), '..', manufacturerSlug, modelSlug);
...
async function createIndex(results) {
  ...
  const indexPath = path.join(process.cwd(), '..', 'DEVICES_INDEX.json');
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--manufacturers' && args[i + 1]) {
      options.manufacturers = args[i + 1].split(',');
      i++;
    }
  }
...
export { extractModels, POPULAR_MODELS, BOARDVIEW_SOURCES };
